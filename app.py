...
2. ✨[현장 감사 관점]: 사용자의 질문이 표면적인 절차 위반(예: 이름만 다르게 보고)이라도, 실제 공장 감사 시 필연적으로 적발되는 파생 위반(예: 성분배합비율 불일치, 생산작업일지 및 원료출납 서류 허위 작성) 관점을 반드시 1개 이상 끌어내어 제안하십시오.
...
3. ✨ [파생 위반 적극 추출]: 질문이 서류/보고와 관련된 편법이라면, DB에서 "서류를 허위로 작성한 경우" 또는 "성분배합비율을 위반한 경우" 등 연관된 실제 처분 원문도 적극적으로 찾아 객관식에 포함하십시오.
...
# [NEW] 최종 출력 프롬프트 (연쇄 파생 위반 리스크 칸 추가)
TEMPLATE = """
...
해당 표면적 위반 외에 현장 실사 시 적발될 수 있는 장부 조작(생산/원료출납 서류 허위 작성)이나 배합비 불일치 조항이 엑셀 DB에 존재하는지 스캔하십시오.
...
| **3. 위반사항 (적용 처분 항목)** | (🚨 경고: 절대 상황을 요약하거나 지어내지 마십시오! Pass 1에서 찾은 엑셀 처분기준표 원문에서 해당되는 공식 조항 문장 전체를 토씨 하나 틀리지 않고 100% 그대로 복사하여 출력하십시오.) |
| **4. 행정처분 수위** | • **1차 처분:** [내용]<br>• **2차 처분:** [내용]<br>• **3차 처분:** [내용] (※ 없으면 '해당 없음') |
| **5. 과태료 및 과징금** | (Pass 1.5에서 확인된 액수, 없으면 '해당 없음') |
| **6. 🚨 [중요] 연쇄 파생 위반 리스크** | (단속반 현장 감사 관점: 표면적 위반 외에 '생산/작업기록 서류 허위 작성', '성분배합비율 위반' 등 연쇄적으로 적발될 수 있는 DB 내 조항과 그 처분 수위(예: 품목류 제조정지 등)를 강력하게 경고하십시오.) |
| **7. 품질관리/대응 가이드** | 1. (대처 방안 1)<br>2. (대처 방안 2)<br>3. (대처 방안 3) |
...
if st.session_state.phase >= 4:
    ...
    # [NEW] 일반 식품(두유 등)과 축산물(우유 등)을 명확히 구분하여 선택하도록 라벨 수정
    biz_choices = [
