...
4. 실무자 지정 구역 최우선: 지정한 [{selected_category}]에 해당하는 범위 안에서만 찾으십시오.
...
# [NEW] 철벽 격리(Isolation) 룰 및 UI 분리 적용
TEMPLATE = """
...
1. 데이터베이스에서 '{selected_case}'와 일치하는 원문을 복사하십시오.
2. ⚠️ [법령 철벽 격리 규칙]: 강제 지정된 업종({business_type})이 '일반 식품'을 포함하면 「축산물 위생관리법」 조항을 절대 출력하지 마십시오. 업종이 '축산물가공업'을 포함하면 일반 「식품위생법」 조항을 절대 출력하지 마십시오. 
3. 조항 번호는 엑셀이나 PDF에 명확히 표기된 팩트 번호만 기재하며, 임의로 짜깁기하지 마십시오.

▶ **[Pass 1.5: 파생 위반 리스크 팩트체크]**
장부 조작 등 연쇄 적발될 수 있는 DB 내 조항을 찾되, 위 [법령 철벽 격리 규칙]을 100% 준수하여 해당 업종에 맞는 법령만 스캔하십시오.
...
| **2. 관련 법령 및 조항** | (DB에서 확인된 정확한 조항 번호 팩트만 기재) |
| **3. 위반사항 (원문)** | (DB 원문 100% 복사) |
| **4. 행정처분 수위** | 1차: [내용] / 2차: [내용] / 3차: [내용] |
| **5. 과태료/과징금** | (DB에서 확인된 액수 팩트만 기재) |
