...
def load_filtered_documents(biz_type):
    ...
    for f in all_files:
        ...
        # [블랙리스트 방식 필터링]
        if "식품제조" in biz_type:
            # 일반 식품업종 선택 시 -> '축산물' 관련 법령만 배제, 나머지는 모두 포함
            if "축산물" not in fname:
                filtered_files.append(f)
                
        elif "축산물" in biz_type:
            # 축산물 업종 선택 시 -> '식품위생법'만 배제, 나머지는 모두 포함
...
🚨 [가독성, 스포일러 방지 및 🎯 AI 추천 규칙] 🚨
1. 실무자가 한눈에 읽을 수 있도록 길고 복잡한 법전 원문 대신 요약된 형태로 3~5개 제안하십시오.
2. ⚠️ [처분 수위 노출 절대 금지]: 5단계는 위반 조항의 '행위'만 명시하는 단계입니다. "영업정지", "품목제조정지", "과태료 O만원" 등의 실제 행정처분 결과는 객관식 보기에 절대 미리 적지 마십시오.
3. 🏢 [업종 철벽 격리]: 강제 지정된 [{business_type}]을 철저히 확인하여 타 업종 조항은 절대 섞지 마십시오.
4. ⚖️ [중대 위반 최우선 탐색]: 내부적으로는 가벼운 위반보다 강력하고 포괄적인 중대 위반 조항을 우선적으로 엑셀에서 스캔하여 보기에 포함하십시오. (출력 시 처분 결과는 숨길 것)
5. 🎯 [AI 초보자 가이드 추천 마크]: 제안하는 3~5개의 항목 중, 사용자의 질문(위반 상황)에 가장 완벽하게 들어맞는 단 하나의 핵심 조항 텍스트 맨 앞에 `[⭐ AI 강력 추천]` 이라는 마크를 반드시 달아주십시오. 법을 모르는 실무자가 이것만 보고도 안심하고 선택할 수 있도록 이끌어 주어야 합니다.
...
실무자가 선택한 요약된 위반 상황('{selected_case}')을 바탕으로, [마스터 통합 데이터베이스]로 다시 돌아가 정확한 팩트를 찾아 리포트를 작성하십시오. (선택지 앞의 [⭐ AI 강력 추천] 텍스트는 검색 시 무시하고 핵심 위반 내용만으로 원문을 찾으십시오.)

🚨 [최종 리포트 도출 3단계 검증 프로세스] 🚨

▶ **[Pass 1: DB 원문 역추적 및 팩트 추출]**
1. 선택된 요약본('{selected_case}')과 의미가 100% 일치하는 실제 엑셀/PDF 원문 데이터를 데이터베이스에서 찾으십시오.
2. ⚠️ [법령 물리적 격리]: [{business_type}]이 '일반 식품'을 포함하면 「축산물 위생관리법」 및 '식육' 관련 조항을 절대 출력하지 마십시오.

▶ **[Pass 1.5: 파생 위반 리스크 팩트체크]**
장부 조작, 자가품질검사 등 연쇄 적발될 수 있는 DB 내 조항을 찾되, 위 [법령 물리적 격리]를 100% 준수하십시오.
...
| **2. 관련 법령 및 조항** | (DB 원문에 명시된 공식 조항 번호만 기재) |
| **3. 위반사항 (원문)** | (역추적하여 찾아낸 DB 내 실제 텍스트 원문 전체 복사. 여기서는 절대 요약 금지) |
| **4. 행정처분 수위** | 1차: [내용] / 2차: [내용] / 3차: [내용] (DB 팩트 기준) |
...
if st.session_state.phase == 5 and st.session_state.case_options:
    ...
    st.info("💡 처분 수위가 가장 무겁고 질문 상황에 가장 적합한 조항에 **[⭐ AI 강력 추천]** 마크가 표시됩니다.")
